...
room_type_df = st.data_editor(
    pd.DataFrame({"部屋タイプ": ["シングル", "ツイン"], "定価(円)": [7500, 9800], "部屋数": [6, 5]}),
    num_rows="dynamic",
    use_container_width=True,
    key="room_type_editor"
)

st.markdown("##### 📅 対象月の日別稼働率")
...
if occ_input_method == "CSVアップロード":
    template_df = pd.DataFrame({
        "日付": [d.strftime("%Y-%m-%d") for d in month_dates],
        "稼働率(%)": [0] * len(month_dates),
    })
    st.download_button(
        "📥 入力用テンプレートCSVをダウンロード",
    ...
    uploaded = st.file_uploader("「日付」「稼働率(%)」の2列を含むCSVをアップロード", type="csv")
    if uploaded is not None:
        try:
            parsed = pd.read_csv(uploaded)
            if "日付" not in parsed.columns or "稼働率(%)" not in parsed.columns:
                st.error("⚠️ CSVには「日付」「稼働率(%)」の列が必要です。テンプレートをご利用ください。")
            else:
                occ_df = parsed
    ...
    occ_df = st.data_editor(default_occ, use_container_width=True, num_rows="fixed", key="occ_editor")
...
def compute_phase_table(occ_data, curve_data):
    """各日について、リードタイム・目標稼働率・実績との差分・フェーズ判定をまとめる"""
    today = date.today()
...
【入力データの内容】
・自社の部屋タイプ別定価
・対象月の日別の稼働ペース（実績稼働率・目標稼働率との差分・フェーズ判定）
・自社の強み・特徴
