...
room_type_df = st.data_editor(
    pd.DataFrame({
        ...
        "定価（日～木）": [12000, 15000, 20000],
        "定価（金・土・祝前日）": [15000, 18000, 28000],
        "最低販売価格（原価ベース・円）": [6500, 10000, 16000],
        "部屋数": [4, 4, 3],
        "ベッド数（シングル）": [0, 4, 0],
        "ベッド数（ダブル）": [1, 0, 0],
        "和式ふとん": [0, 0, 2],
...
with col5:
    current_promotions = st.text_area(
        "現在行っているプロモーション（期間限定割引）",
        placeholder="例：9月限定 2連泊で15%OFF\n早期予約28日前割引 10%OFF",
        height=100,
    )
with col6:
...
・自社の強み・特徴
・現在実施中のプロモーション・アメニティ施策、および過去に実施した施策とその効果
...
if st.button("🚀 AIで月次収益改善策を生成する", type="primary"):
    if st.session_state.hotel_df is None:
        ...
        user_message = (
            ...
            f"【自社の強み・特徴】\n{own_strengths_str}\n\n"
            f"【現在実施中のプロモーション（期間限定割引）】\n{current_promotions or '特になし'}\n\n"
